-- PostgREST returns composite row types as "(uuid,...)" strings — clients expect JSON.
-- Return jsonb from account RPCs so register/login/get return proper objects.
-- Must DROP first: PostgreSQL cannot change return type via CREATE OR REPLACE.

drop function if exists public.escape_register_account(text, text, text);
drop function if exists public.escape_login_account(text, text);
drop function if exists public.escape_get_account(uuid);
